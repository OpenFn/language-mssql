...
function upsert(table, uuid, record) {
  return function (state) {
    ...
    var _expandReferences2 = (0, _languageCommon.expandReferences)(record)(state),
        recordData = _expandReferences2.recordData;
...
function insert(table, record) {
  return function (state) {
    ...
    var _expandReferences3 = (0, _languageCommon.expandReferences)(record)(state),
        recordData = _expandReferences3.recordData;

    var columns = Object.keys(recordData).sort();
    var values = columns.map(function (key) {
      return recordData[key];
    }).join(', ');
    var query = "INSERT INTO ".concat(table, " (").concat(columns.join(', '), ") VALUES (").concat(values, ");");
    return new Promise(function (resolve, reject) {
      console.log("Executing upsert via SQL 'MERGE' function.");
      var request = new _tedious.Request(query, function (err, rowCount, rows) {
        if (err) {
...
function insertMany(table, record) {
  return function (state) {
    var connection = state.connection;

    var _expandReferences4 = (0, _languageCommon.expandReferences)(record)(state),
        recordData = _expandReferences4.recordData; // Note: we select the keys of the FIRST object as the canonical template.
    ...
    var query = "INSERT INTO ".concat(table, " (").concat(columns.join(', '), ") VALUES ").concat(valueSets.join(', '), ";");
    return new Promise(function (resolve, reject) {
      console.log("Executing upsert via SQL 'MERGE' function.");
      var request = new _tedious.Request(query, function (err, rowCount, rows) {
        if (err) {
